...
export class GraphService {
  ...
  async getUserEmail(): Promise<string> {
    ...
    try {
      const response = await this.makeRequest("https://graph.microsoft.com/v1.0/me")
  ...
  async getAllEmails(top = 50): Promise<Email[]> {
    try {
      const [inboxEmails, sentEmails] = await Promise.all([this.getEmails(top), this.getSentEmails(top)])

      const allEmails = [...inboxEmails, ...sentEmails]
      return allEmails.sort((a, b) => new Date(b.receivedDateTime).getTime() - new Date(a.receivedDateTime).getTime())
    } catch (error) {
  ...
  groupEmailsIntoThreads(emails: Email[]): EmailThread[] {
    ...
    emails.forEach((email) => {
      ...
      const threadKey = email.conversationId || this.normalizeSubject(email.subject || "")
    ...
    const threads = Array.from(threadsMap.values()).map((thread) => {
      thread.emails.sort((a, b) => new Date(a.receivedDateTime).getTime() - new Date(b.receivedDateTime).getTime())
  ...
  private getCleanSubject(subject: string): string {
    return subject.replace(/^(Re:|RE:|Fwd:|FWD:|Fw:|FW:)\s*/gi, "").trim() || "(Sem assunto)"
  }
  ...
  async replyToEmail(emailId: string, replyDraft: Partial<EmailDraft>): Promise<void> {
    await this.makeRequest(`https://graph.microsoft.com/v1.0/me/messages/${emailId}/reply`, {
      method: "POST",
      body: JSON.stringify({
        message: replyDraft,
      }),
  ...
  async replyAllToEmail(emailId: string, replyDraft: Partial<EmailDraft>): Promise<void> {
    await this.makeRequest(`https://graph.microsoft.com/v1.0/me/messages/${emailId}/replyAll`, {
      method: "POST",
      body: JSON.stringify({
        message: replyDraft,
      }),
  ...
  async forwardEmail(emailId: string, forwardDraft: Partial<EmailDraft>): Promise<void> {
    await this.makeRequest(`https://graph.microsoft.com/v1.0/me/messages/${emailId}/forward`, {
      method: "POST",
      body: JSON.stringify({
  ...
  async markAsRead(emailId: string): Promise<void> {
    await this.makeRequest(`https://graph.microsoft.com/v1.0/me/messages/${emailId}`, {
      method: "PATCH",
      body: JSON.stringify({
        isRead: true,
      }),
  ...
  async moveToFolder(emailId: string, folderId: string): Promise<void> {
    await this.makeRequest(`https://graph.microsoft.com/v1.0/me/messages/${emailId}/move`, {
      method: "POST",
      body: JSON.stringify({
